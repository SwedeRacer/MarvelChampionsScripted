    ...
    for _, playerColor in pairs(playerColors) do
        if (selectedHeroes[playerColor] ~= nil) then
            table.insert(players, {
                ...
                hero = selectedHeroes[playerColor]
            })
        end
    end
...
function placeRandomHero(params)
    ...
    local heroList = getHeroesByTeam({
        team = params.team
    })
...
function placeHeroWithImportedDeck(params)
    ...
    local heroKey = findHeroKeyByIdentityId({
        identityId = heroIdentityId
    })
...
function placeHero(heroKey, playerColor, deckType, importedDeck)
    if not confirmPlayerIsSeated(playerColor) then
        return
    end
    if not confirmSeatIsAvailable(playerColor) then
        return
    end

    local hero = deepCopy(heroes[heroKey])
    hero.key = heroKey
    local playmatPosition = getPlaymatPosition({
        playerColor = playerColor
    })

    placePlaymat(playerColor, playmatPosition, hero.playmatImageUrl)

    placeHealthCounter(playerColor, playmatPosition, hero.tileImageUrl, hero.hitPoints)

    placeIdentity(hero, playmatPosition, playerColor)

    placeDeck(hero, deckType, playmatPosition, importedDeck, playerColor)

    Wait.frames(function()
        placeCards({
            hero = hero,
            playerColor = playerColor,
            playmatPosition = playmatPosition
        })
    end, 30)

    Wait.frames(function()
        placeExtras(hero, playmatPosition, playerColor)
    end, 120)
...
function clearHero(params)
    ...
    local playerControl = getPlayerControl({
        playerColor = params.playerColor
    })

    playerControl.call("createSuitUpButton", {
        skipScenarioCheck = skipScenarioCheck
    })
end
...
function getPlayerDeckPositions(params)
    ...
    if (params.includeDeck ~= nil) then
        includeDeck = params.includeDeck
    end
    if (params.includeDiscard ~= nil) then
        includeDiscard = params.includeDiscard
    end
...
function getAllPlayerDeckPositions(params)
    ...
    for _, hero in pairs(selectedHeroes) do
        local playerDeckPositions = getPlayerDeckPositions({
            hero = hero,
            includeDeck = includeDeck,
...
function configurePlaymat(params)
    ...
    Wait.frames(function()
        ...
        reloadedPlaymat.call("setPlayerColor", {
...
function configureHealthCounter(params)
    ...
    counter.addTag("health-counter-" .. params.playerColor)
...
function placeCards(params)
    local hero = params.hero
    local playerColor = params.playerColor
    local playmatPosition = params.playmatPosition

    if (hero.cards == nil) then
        return
    end

    function placeCardCoroutine()
    ...
    startLuaCoroutine(self, "placeCardCoroutine")
end
...
function findAndPlacePlayerCard(params)
    ...
    local deckPositions = getPlayerDeckPositions({
        hero = hero
    })
    local decks = {}
...
function layOutHeroSelectors(params)
    local heroList = params.team ~= nil and getHeroesByTeam({
        team = params.team
    }) or heroes
    local layoutManager = getObjectFromGUID(Global.getVar("GUID_LAYOUT_MANAGER"))
...
function setFirstPlayer(params)
    ...
    scenarioManager.call("adjustTurnCount", {
        adjustment = 1
    })
